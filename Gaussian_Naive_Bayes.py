...
for i in range(0, X_test.shape[0]):
	P_X = np.divide(np.exp(-1*(np.divide(np.power(np.subtract(X_test[i, :].reshape(X_test.shape[1], 1), mean), 2), 2*np.power(std, 2)))), math.sqrt(2*np.pi)*std)																		# Compute independent probabilities of features w.r.t. class
	Class_X = np.argmax(logP_class+np.sum(np.nan_to_num(np.log(P_X)), axis=0))		# Compute class prediction for the test sample 	
	pred.append(Class_X)
